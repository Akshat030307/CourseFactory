...
**[P1] Mastery overlay** — tint graph nodes by `mastery.score`. Turns the diagram into a dashboard.
**[P2] Deep links** — `/lecture/3?t=884000&lane=board`, restoring player position and lane.
**[P3] Keyboard nav** — arrows step board states, `/` focuses search, `Esc` returns to origin.
**[P4] Accessibility floor** — visible focus rings, reduced motion, mobile down to 380px.
**[P5] Empty and error states** — say what to do, not what failed.

---

## Stage 11 — Demo readiness

**[Z1] Pre-index the fixtures** — all four lectures fully processed and verified before demo time. Keep a database dump so a bad run can be restored in seconds.

**[Z2] Record a backup video** — the full four-beat run, screen-recorded. Do this while things work, not at the end.

**[Z3] Rehearse the failure paths** — kill the network and confirm the app still serves pre-indexed content. Force a Groq 429 and confirm the OpenAI fallback fires silently.

**[Z4] Time the run** — the four beats should take under 100 seconds. Rehearse out loud three times.
