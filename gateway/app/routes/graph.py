...
async def get_course_graph(request: Request, course_id: str, lecture_id: str | None = None) -> Graph:
    ...
    async with request.app.state.pool.acquire() as conn:
        if lecture_id is None:
            node_rows = await conn.fetch(load("graph_nodes.sql"), course_id, DEMO_STUDENT_ID, None)
            ...
            node_rows = await conn.fetch(load("graph_nodes.sql"), course_id, DEMO_STUDENT_ID, all_ids)
