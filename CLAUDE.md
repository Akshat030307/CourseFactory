...
- Don't build multi-tenancy, user registration, or per-student accounts. Stage 13 added real *login* (see `docs/DEPLOYMENT.md`, `gateway/app/auth.py`) because a public VPS URL can't be wide open — but it's still one account gating the same one-student/one-instructor model, not a rebuild into multi-user SaaS. That's a materially bigger, unasked-for project.
